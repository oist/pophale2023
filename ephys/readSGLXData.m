%filename is a binary file acquired with spikeGLX (imec or NI)

%ch is channel numbers to apply gain correction to (1 based). Other
%channels will be loaded but not gain corrected

%sSec is start time of data to pull, in sec
%nSec is number of seconds to pull


function [dataArray,tVec, meta]=readSGLXData(fileName,ch,sSec,nSec)
...
sSamp = floor(sSec * SampRate(meta));
nSamp = floor(nSec * SampRate(meta));

dataArray = ReadBin(sSamp, nSamp, meta, binName, path);
...
if dataType == 'A'
    ...
    tVec=(sSamp:(sSamp+nSamp-1))*1/SampRate(meta)+sSec;
else
